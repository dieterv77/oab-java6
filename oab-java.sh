...
# References
#  - git://github.com/rraptorr/sun-java6
...
function usage() {
    ...
    echo
    echo "* git://github.com/rraptorr/sun-java6"
    echo "* git://github.com/rraptorr/oracle-java7"    
    echo
    ...
    echo "* Clone the build scripts from git://github.com/rraptorr/"
...
ncecho " [x] Removing clones of git://github.com/rraptorr/${JAVA_UPSTREAM} "
rm -rfv ${WORK_PATH}/${JAVA_UPSTREAM}* 2>/dev/null >> "$log" 2>&1
rm -rfv ${WORK_PATH}/src 2>/dev/null >> "$log" 2>&1 &
pid=$!;progress $pid

# Clone the code
ncecho " [x] Cloning git://github.com/rraptorr/${JAVA_UPSTREAM} "
cd ${WORK_PATH}/ >> "$log" 2>&1
git clone git://github.com/rraptorr/${JAVA_UPSTREAM} src >> "$log" 2>&1 &
pid=$!;progress $pid
...
BUILD_MESSAGE="Automated build for ${LSB_REL} using git://github.com/rraptorr/${JAVA_UPSTREAM}"
